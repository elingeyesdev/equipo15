...
export const ModerationModals = () => {
  ...
  useEffect(() => {
    ...
    const softHandler = (payload: any) => {
      setWarningData(payload);
      setProfile({ ...userProfile, status: 'SOFT_BLOCK' } as any);
    };

    const suspendedHandler = (payload: any) => {
      setPenaltyData(payload);
      setProfile({ ...userProfile, status: 'SUSPENDED' } as any);
    };

    socket.on(`user:soft_block:${userProfile.id}`, softHandler);
    socket.on(`user:suspended:${userProfile.id}`, suspendedHandler);

    return () => {
      socket.off(`user:soft_block:${userProfile.id}`, softHandler);
      socket.off(`user:suspended:${userProfile.id}`, suspendedHandler);
    };
  }, [user, userProfile?.id, setProfile, socket]);
