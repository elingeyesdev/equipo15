...
function buildValidDto(
  ...
  dto.companyContext =
    'Somos una universidad grande con miles de estudiantes activos en diferentes facultades.';
  dto.participationRules =
    'Equipos de máximo 5 personas. Solo estudiantes regulares pueden participar.';
...
describe('CreateChallengeDto — Contenido y Calidad', () => {
  ...
  it('debe fallar si el título es puro texto repetitivo (spam)', async () => {
    const dto = buildValidDto({
      title: 'asdf asdf asdf asdf asdf asdf asdf asdf asdf asdf',
    });
    const errors = await validate(dto);
    const titleError = errors.find((e) => e.property === 'title');
    expect(titleError).toBeDefined();
  ...
  it('debe fallar si el título contiene una URL insegura (http://)', async () => {
    const dto = buildValidDto({
      title: 'Visita http://sitio-malicioso.com para más información',
    });
    const errors = await validate(dto);
    const titleError = errors.find((e) => e.property === 'title');
    expect(titleError).toBeDefined();
  ...
  it('debe pasar si el título contiene una URL segura (https://)', async () => {
    const dto = buildValidDto({
      title:
        'Consulta en https://empresa-segura.com nuestra convocatoria oficial',
    });
    const errors = await validate(dto);
    const urlError = errors.find(
      (e) =>
        e.property === 'title' &&
        e.constraints &&
        'noInsecureUrls' in e.constraints,
    );
...
describe('CreateChallengeDto — Números y Símbolos', () => {
  it('debe fallar si el título contiene números', async () => {
    const dto = buildValidDto({
      title: 'Reto número ciento veintitrés para estudiantes',
    });
    ...
    const titleError = errors.find(
      (e) =>
        e.property === 'title' && e.constraints && 'noNumbers' in e.constraints,
    );
    expect(titleError).toBeDefined();
  });
  ...
  it('debe fallar si la descripción contiene números', async () => {
    ...
    dto.problemDescription = dto
      .problemDescription!.replace('cinco', '5')
      .replace('doscientos', '200');
    const errors = await validate(dto);
    const descError = errors.find(
      (e) =>
        e.property === 'problemDescription' &&
        e.constraints &&
        'noNumbers' in e.constraints,
    );
    expect(descError).toBeDefined();
  });
  ...
  it('debe fallar si el contexto de empresa contiene números', async () => {
    const dto = buildValidDto({
      companyContext: 'Tenemos 500 empleados en toda la región.',
    });
    const errors = await validate(dto);
    const ctxError = errors.find(
      (e) =>
        e.property === 'companyContext' &&
        e.constraints &&
        'noNumbers' in e.constraints,
    );
  ...
  it('debe pasar si el título no tiene números ni símbolos raros', async () => {
    const dto = buildValidDto({
      title: 'Rediseñar la experiencia del estudiante universitario',
    });
    const errors = await validate(dto);
    const titleError = errors.find(
      (e) =>
  ...
  it('debe fallar si el título es spam de símbolos especiales', async () => {
    ...
    const titleError = errors.find(
      (e) =>
        e.property === 'title' &&
        e.constraints &&
        'noExcessiveSymbols' in e.constraints,
    );
    expect(titleError).toBeDefined();
  });
  ...
  it('debe permitir números en participationRules (sin NoNumbers)', async () => {
    const dto = buildValidDto({
      participationRules:
        'Equipos de máximo 5 personas, estudiantes de semestre 3 en adelante.',
    });
    const errors = await validate(dto);
    const rulesError = errors.find(
      (e) =>
        e.property === 'participationRules' &&
        e.constraints &&
        'noNumbers' in e.constraints,
    );
  ...
  it('debe fallar si participationRules es solo spam de números', async () => {
    const dto = buildValidDto({
      participationRules: '1234567890123456789012345678901234567890',
    });
    const errors = await validate(dto);
