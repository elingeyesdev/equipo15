...
const assertTextQuality = (value: string, fieldLabel: string): void => {
  ...
  if (isOnlyNumbers(value)) {
    buildValidationError(fieldLabel, 'El comentario no puede contener solo números.');
  }
  ...
  if (repeatedCharCount > 0) {
    buildValidationError(fieldLabel, 'El comentario contiene spam o letras repetidas de forma excesiva.');
  }

  const repeatedWordCount = (value.match(REPEATED_WORD_PATTERN) || []).length;
  if (repeatedWordCount > 0 || KEYBOARD_SPAM_PATTERN.test(value) || REPEATED_SEQUENCE_PATTERN.test(value)) {
  ...
  if (words.length >= 4 && uniqueWords.size / words.length <= 0.5) {
    buildValidationError(fieldLabel, 'El comentario contiene demasiado texto repetido.');
  }
  ...
  if (letters >= 8 && uppercaseRatio >= COMMENT_CONTENT_RULES.maxUppercaseRatio && !isAcronymLike(value)) {
    buildValidationError(fieldLabel, 'Evita escribir el comentario completamente en mayúsculas.');
  }
};
...
const assertWhitespaceRules = (rawValue: string, fieldLabel: string): void => {
  ...
  if (consecutiveBreaksRegex.test(normalizedNewlines)) {
    buildValidationError(fieldLabel, 'El comentario tiene demasiados saltos de línea consecutivos.');
  }
};
